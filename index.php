<?php get_header(); ?>
        <div class="mainslider glide">
            <div class="glide__track" data-glide-el="track">
                <ul class="glide__slides">
                    <!-- слайдер в виде цикла через посты в админке -->
                    <?php
                        // параметры по умолчанию
                        $my_posts = get_posts( array(
                            'numberposts' => -1,
                            'category_name'    => 'slider', //указываем категорию, откуда будут браться the_title и прочее
                            'orderby'     => 'date',
                            'order'       => 'ASC',
                            'post_type'   => 'post',
                ...
                <div class="row">
                    <div class="col-md-10 offset-md-1 col-lg-5 offset-lg-1">
                        <div class="about__img">
                            <!-- 2 варианта вывода картинки: из массива данных и просто ссылкой. Настраивается в кастом филдс -->
                            ...
                            print_r($image);
                            print_r('<br><br>ЭТО ВЫВОДИТСЯ СПОМОЩЬЮ print_r')
                            ?>
                        </div>
                    </div>
        ...
        <div class="toys" id="toys">
            <div class="container">
                <!-- Мягкие игрушки -->
                <!-- вывел название рубрики -->
                <h2 class="subtitle"><?= get_the_category_by_ID('3'); ?></h2>
                ...
                <!-- Развивающие игрушки -->
                <h2 class="subtitle"><?= get_the_category_by_ID('4'); ?></h2>
                <div class="toys__wrapper">
                <?php
                    $my_posts = get_posts( array(
                        'numberposts' => -1,
        ...
        <div class="contacts" id="contacts">
            <h1 class="title"><?php the_field('coordinations_title'); ?></h1>
            <div class="container">
                <div class="row">
                    <div class="col-lg-6">
                        <div class="contacts__descr underlined">
                        <?php the_field('coordinations_address'); ?>
                        </div>
                    </div>
                    <div class="col-lg-6">
                        <div class="contacts__map">
                            <!-- через ACF не работает, просто скопировал код из карт -->
                            <iframe src="https://www.google.com/maps/embed?pb=!1m14!1m8!1m3!1d253829.49231320733!2d-75.569131!3d6.252423000000001!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x8e4428feb81e9f5d%3A0x213a72a1fad66b8b!2sMuseo%20de%20Antioquia!5e0!3m2!1ses-419!2smx!4v1709684892615!5m2!1ses-419!2smx" width="100%" height="100%" style="border:0;" allowfullscreen="" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>
                        </div>
                    </div>
                </div>
                <div class="row">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="contacts__feed">
                            <?= do_shortcode( '[contact-form-7 id="1e459f2" title="Форма отзывов"]' ); ?>
                        </div>
                    </div>
                </div>
                ...
                <div class="row mt70">
                    <div class="col-lg-8 offset-lg-2">
                        <div class="title"><?= get_the_category_by_ID('5'); ?></div>
...
<?php 
    function print_hello($text, $name) {
        echo 'Hello, world!' . '' . $text . ' ' . $name; // выводим текст с аргументами
    };

    add_action( 'my_hook', 'print_hello', 10, 2); // запускаем функцию print_hello вместе с my_hook, приоритет 10, 2 входных аргумента $text и $name

    do_action( 'my_hook', ' dear customer,', 'Elena' ); // запускаем хук my_hook, передаем ргументы для print_hello - dear customer и Elena
?>

<!-- фильтр -->
<?php
    function my_filter_function($str) {
        return '<br>' . 'Hello ' . $str; // выводим текст
    };
    add_filter( 'my_filter', 'my_filter_function', 15); 

    echo apply_filters( 'my_filter', 'world!!!' ); // сначала запускается my_filter, потом идем вверх, запускаем вместе с ним my_filter_function

    remove_filter(  'my_filter', 'my_filter_function', 15 ); // отцепил функцию от хука

    echo apply_filters( 'my_filter', '<br> world!!!' ); // поэтому вывел только world!!!. Так же работает remove_action()
?>
